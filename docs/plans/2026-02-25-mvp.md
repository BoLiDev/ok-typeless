...
**Deliverable:** `useAudioRecorder` hook that manages `getUserMedia` + `MediaRecorder` lifecycle, sends the assembled `ArrayBuffer` to main via `window.typeless.sendAudioData`.
...
- `onStartRecording` IPC → calls `getUserMedia`, starts `MediaRecorder`
- `onStopRecording` IPC → stops recorder, assembles chunks into `ArrayBuffer`, calls `sendAudioData`
- On `getUserMedia` success: notifies main that mic is ready (state machine advances to `recording`)
- On `getUserMedia` failure: notifies main (state machine advances to `error`)
- 5-second timeout for `getUserMedia` → transitions to `error("Mic timeout")`

**Back pressure:** TypeScript types enforce the IPC contract. Manual test via full flow.
